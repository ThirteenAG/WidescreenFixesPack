...
<img src="https://thirteenag.github.io/screens/re1/main2.jpg" width="760" alt="Resident Evil Fusion Fix">

**Resident Evil Fusion Fix** unlocks display resolutions and offers an optional FPS unlock, door skipping, cursor hiding, visual filter toggles, and Logitech RGB lighting.

[Website](https://thirteenag.github.io/wfp#re1) · [Source Code](https://github.com/ThirteenAG/WidescreenFixesPack/blob/master/source/ResidentEvil.FusionFix/dllmain.cpp) · [Default INI](https://github.com/ThirteenAG/WidescreenFixesPack/blob/master/data/ResidentEvil.FusionFix/scripts/ResidentEvil.FusionFix.ini)
...
- **Frame rate** — Enable `UnlockFPS` and set **FRAME RATE** to **VARIABLE** to raise the frame rate cap.
- **Resolution list** — Replace the predefined list, previously capped at 2560×1600, with available display resolutions.
- **Mouse cursor** — Hide the cursor after a configurable interval.
- **Door animations** — Skip door transitions.
- **Windowed mode** — Enable borderless windowed mode.
- **Color correction** — Optionally disable color correction.
- **Noise filter** — Optionally disable the noise filter.
...
3. Optionally edit `scripts/ResidentEvil.FusionFix.ini` to configure the fix.
4. Launch the game.

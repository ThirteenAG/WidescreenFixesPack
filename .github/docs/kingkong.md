...
$${\color{green}✔}$$  Fixed gamepad support in main game and settings application
...
$${\color{blue}ⓘ}$$ To make Gamer's Edition load faster, download [KingKongGamersEdition.TextureCache.zip](https://github.com/ThirteenAG/WidescreenFixesPack/releases/download/kingkong/KingKong.WidescreenFix.zip) and place `Textures` folder to the game root directory. Without it, the texture cache will be generated by Widescreen Fix on each loading screen and will only make subsequent loading faster

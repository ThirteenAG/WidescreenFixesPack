...
def darken_image(path, relpath):
    ...
    data = img.getdata()
    gamma = 1.0/1.5
    new_pixels = list(map(lambda p: (int(pow(p[0]/255, (1/gamma))*255), int(pow(p[1]/255, (1/gamma))*255), int(pow(p[2]/255, (1/gamma))*255), p[3]), data))
    img.putdata(new_pixels)
    img.save(path)
    print("  darkened!")
    return True
...
def darken_images(idir, force):
    ...
    for relpath in list(scaled_images):
        if darken_image(os.path.join(idir, relpath), relpath):
            scaled_images.remove(relpath)
    put_scaled_images(idir, scaled_images)
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", type=str,
            help="Command, should be scale or unscale or darken")
    ...
    if args.command == "scale":
        ...
        sys.exit(0)
    elif args.command == "darken":
        darken_images(args.directory, args.force)
        sys.exit(0)
    else:

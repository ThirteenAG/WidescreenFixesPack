...
DWORD WINAPI Init(LPVOID)
{
	...
	}; injector::MakeInline<ResHook>(pattern.get(0).get<uintptr_t>(0), pattern.get(0).get<uintptr_t>(9));
	...
	}; injector::MakeInline<HudHook>(pattern.get(0).get<uintptr_t>(0), pattern.get(0).get<uintptr_t>(6));
	...
	}; injector::MakeInline<ClickableAreaHook>(pattern.get(0).get<uintptr_t>(0), pattern.get(0).get<uintptr_t>(7));

	pattern = hook::pattern("D8 25 ? ? ? ? D8 F2 D9 5C 24 2C D9"); //0x4E0BC4
	injector::WriteMemory(pattern.get(0).get<uintptr_t>(2), &Screen.fTextOffset, true);
	...
	}; injector::MakeInline<TextHook>(pattern.get(0).get<uintptr_t>(0), pattern.get(0).get<uintptr_t>(6));

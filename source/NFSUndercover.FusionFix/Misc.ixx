...
void Init4()
{
    ...
    }; injector::MakeInline<GPSMapBackgroundHook2>(pattern.count(1).get(0).get<uint32_t>(0), pattern.count(1).get(0).get<uint32_t>(6)); // 83037F

    // Transition Overlay Width
    pattern = GetPattern("E8 ? ? ? ? D9 5C 24 14 E8 ? ? ? ? D9 5C 24 1C");
    if (pattern.empty())
        pattern = GetPattern("E8 ? ? ? ? ? ? ? ? E8 ? ? ? ? ? ? ? ? 0F B6 44 24");
    ...
    uint32_t* dword_76B7FF = find_pattern("74 ? 6A ? B9 ? ? ? ? E8 ? ? ? ? F3 0F 2A C0", "74 ? 6A ? B9 ? ? ? ? E8 ? ? ? ? F3 0F 2A C0").get_first<uint32_t>();
    injector::WriteMemory<uint8_t>(dword_76B7FF, 0xEB, true); // jmp
    uint32_t* dword_74A81D = find_pattern("D9 C9 DF F1 DD D8 76 ? F2 0F 10 0D", "D9 C9 DF F1 DD D8 76 ? F3 0F 10 0D").count(1).get(0).get<uint32_t>(6);
    injector::MakeNOP(dword_74A81D, 2, true); // 2 nops
    auto dword_76B84C = hook::pattern("F3 0F 10 05 ? ? ? ? F3 0F 11 04 24 D9 04 24 83 C4 08");
    if (!dword_76B84C.empty())
        injector::WriteMemory(dword_76B84C.count(1).get(0).get<uint32_t>(4), &fVertFOV, true); // 2 nops
    else
    {
        dword_76B84C = hook::pattern("C3 F3 0F 10 05 ? ? ? ? ? ? ? ? ? ? ? ? 59 C3");
        injector::WriteMemory(dword_76B84C.count(3).get(2).get<uint32_t>(5), &fVertFOV, true); // 2 nops
    }
    ...
    {
        pattern = GetPattern("F3 0F 10 44 24 ? F3 0F 5E 44 24 ? F3 0F 11 44 24");
        struct FOVHook
        {
            void operator()(injector::reg_pack& regs)
            {
                float esp28 = *(float*)(regs.esp + 0x28);
                *(float*)(regs.esp + 0x28) = (esp28 * fHudScale);
                esp28 = *(float*)(regs.esp + 0x28);
                regs.xmm0.f32[0] = esp28;
            }
        }; injector::MakeInline<FOVHook>(pattern.get_first(0), pattern.get_first(6));
    }
    ...
    pattern = find_pattern("F3 0F 11 0D ? ? ? ? 0F 5A C8", "F3 0F 11 0D ? ? ? ? 56");
    static float* CSM_fCSMScaleLevel = *pattern.get_first<float*>(4);
    // disable writes to the array -- MIGHT BREAK CUTSCENES as they have their own scales!
    injector::MakeNOP(pattern.get_first(), 8);

    pattern = find_pattern("F3 0F 11 0D ? ? ? ? F3 0F 11 05 ? ? ? ? E8 ? ? ? ? 8B C8", "F3 0F 11 0D ? ? ? ? F3 0F 11 05 ? ? ? ? E8 ? ? ? ? 8B C8");
    injector::MakeNOP(pattern.get_first(0), 8);
    injector::MakeNOP(pattern.get_first(8), 8);
    ...
    pattern = hook::pattern("77 ? F3 0F 5A C0 0F 5A C9");
    if (!pattern.empty())
    {
        injector::MakeJMP(pattern.get_first(), pattern.get_first(15), true);
    }
    else
    {
        pattern = hook::pattern("77 ? 0F 2F CA 77 ? B0 01");
        injector::MakeJMP(pattern.get_first(), pattern.get_first(7), true);
    }
    ...
    if (bImproveSceneryLOD)
    {
        ...
        {
            pattern = hook::pattern("C6 44 24 ? ? 75 ? C6 44 24 ? ? 83 BF");
            uint32_t* dword_82B0AA = pattern.count(1).get(0).get<uint32_t>(4);
            injector::WriteMemory<uint8_t>(dword_82B0AA, 0x0F, true);
        }
    }
    ...
    if (bDisablePreculler)
    {
        ...
        pattern = find_pattern("8B 0D ? ? ? ? 85 C9 B3 ? 0F 84", "8B 0D ? ? ? ? 85 C9 B3 ? 74 ? 83 F9");
        uint32_t* PrecullerMode = *pattern.count(1).get(0).get<uint32_t*>(2);
        *PrecullerMode = 0;
    }
    ...
    if (SimRate)
    {
        ...
        auto pattern = hook::pattern("F2 0F 5E 05 ? ? ? ? 66 0F 5A C8 F3 0F 10 44 24 ? 0F 5A D0 F3 0F 5A D9 66 0F 2F DA 77 ? 0F 28 C1 EB ? F3 0F 10 44 24"); //0x6ADF85 anchor, 0x6ADFEB pointer write
        if (!pattern.empty())
            injector::WriteMemory(pattern.get_first(4), &dSimRate, true);
        else
        {
            static float fSimRate = 1.0f / (float)SimRate;
            pattern = hook::pattern("F3 0F 59 0D ? ? ? ? 0F 2F C8 77 ? 0F 28 C1 83 3D");
            injector::WriteMemory(pattern.get_first(4), &fSimRate, true);
        }

        // MainLoop double frametime (FPS lock) .rdata
        pattern = hook::pattern("F2 0F 59 05 ? ? ? ? F2 0F 5A C0 F3 0F 11 44 24 ? 80 3D");//0x6795D0 anchor
        if (!pattern.empty())
            injector::WriteMemory<double>(*pattern.get_first<double*>(4), dFrameTime, true); //0x679754 location dereference
        else
        {
            static float fFrameTime = 1.0f / (float)SimRate;
            pattern = hook::pattern("F3 0F 10 0D ? ? ? ? 74 ? F3 0F 2A C0");
            injector::WriteMemory<float>(*pattern.get_first<float*>(4), fFrameTime, true);
        }
        ...
        pattern = find_pattern("F2 0F 10 15 ? ? ? ? F3 0F 5A C8 66 0F 2F D1 76 ? F3 0F 10 05 ? ? ? ? 80 7E"); //0x83C6CD anchor
        if (!pattern.empty())
            injector::WriteMemory<double>(*pattern.get_first<double*>(4), -dFrameTime, true); //0x679754 location dereference
        else
        {
            pattern = hook::pattern("F3 0F 10 15 ? ? ? ? 77 ? 0F 28 C1 EB 08 0F 2F D0 76 ? 0F 28 C2 80 7E");
            injector::WriteMemory<float>(*pattern.get_first<float*>(4), float(-dFrameTime), true);
        }

        // Unknown frametime 3 .rdata
        pattern = find_pattern("F3 0F 10 05 ? ? ? ? 80 7E ? ? 8B 47"); //0x83C6FE location dereference
        if (!pattern.empty())
            injector::WriteMemory<float>(*pattern.get_first<float*>(4), -FrameTime, true);

        // Sim::QueueEvents frametime .rdata (this affects gameplay smoothness noticeably)
        pattern = find_pattern("F3 0F 10 15 ? ? ? ? 0F 2F D1 76 ? F3 0F 11 0D", "F3 0F 10 15 ? ? ? ? 0F 2F D0 76 ? F3 0F 11 05"); //0x7B89BC anchor, 0x7B89E4 location dereference
        injector::WriteMemory<float>(*pattern.get_first<float*>(4), FrameTime, true);
...
public:
    Misc()
    {
        WFP::onInitEvent() += []()
        {
            ...
            CallbackHandler::RegisterCallbackAtGetSystemTimeAsFileTime(Init1, hook::range_pattern(ModuleStart, RangeEnd, "C7 44 24 ? ? ? ? ? FF 15 ? ? ? ? 8D 54 24 0C 52"));
            CallbackHandler::RegisterCallbackAtGetSystemTimeAsFileTime(Init2, hook::range_pattern(ModuleStart, RangeEnd, "E8 ? ? ? ? 8B 4E 04 83 C4 20 89 46 08 8B 15 ? ? ? ? 51 52 68 0B 20 00 00"));
            CallbackHandler::RegisterCallbackAtGetSystemTimeAsFileTime(Init4, hook::range_pattern(ModuleStart, RangeEnd, "8B 0D ? ? ? ? 6A 01 51 8B C8"));
        };
    }
} Misc;
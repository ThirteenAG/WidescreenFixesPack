...
{
    float w;
    if (CDraw::GetAspectRatio() < fDefaultWidth)
        w = static_cast<float>(RsGlobal->maximumWidth);
    else
        w = RsGlobal->maximumWidth * fDefaultWidth / CDraw::GetAspectRatio();
...
{
    hbDrawRect.fun(CRect(fDefaultCoords + rect.left * fDefaultWidth / CDraw::GetAspectRatio(), rect.bottom, fDefaultCoords + rect.right * fDefaultWidth / CDraw::GetAspectRatio(), rect.top), color);
}
...
void __fastcall DrawSpriteHook(CSprite2d const& sprite, int, CRect const& rect, CRGBA const& color)
{
    ...
    {
        hbDraw.fun(sprite, -1, CRect(fDefaultCoords + rect.left * fDefaultWidth / CDraw::GetAspectRatio(), rect.bottom, fDefaultCoords + rect.right * fDefaultWidth / CDraw::GetAspectRatio(), rect.top), color);

        if (rect.left <= 0.0f && rect.right >= (float)RsGlobal->maximumWidth * 320.0f * 0.0015625)
        {
            CSprite2dDrawRect(CRect(0.0f, (float)RsGlobal->maximumHeight, fDefaultCoords, 0.0f), CRGBA(0, 0, 0, 255));

            CSprite2dDrawRect(CRect(fDefaultCoords + (float)RsGlobal->maximumWidth * fDefaultWidth / CDraw::GetAspectRatio(), (float)RsGlobal->maximumHeight, (float)RsGlobal->maximumWidth, 0.0f), CRGBA(0, 0, 0, 255));
        }
    }
    else
    {
...
{
    hbDraw2.fun(sprite, 0, fDefaultCoords + x1 * fDefaultWidth / CDraw::GetAspectRatio(), y1, fDefaultCoords + x2 * fDefaultWidth / CDraw::GetAspectRatio(), y2,
                fDefaultCoords + x3 * fDefaultWidth / CDraw::GetAspectRatio(), y3, fDefaultCoords + x4 * fDefaultWidth / CDraw::GetAspectRatio(), y4, color);
}
...
{
    hbPrintString.fun(fDefaultCoords + x * fDefaultWidth / CDraw::GetAspectRatio(), y, str);
}

injector::hook_back<void(__cdecl*)(float, float)> hbSetScale;
void __cdecl SetScaleHook(float w, float h)
{
    hbSetScale.fun(w * fDefaultWidth / CDraw::GetAspectRatio(), h);
}

injector::hook_back<void(__cdecl*)(float)> hbSetWrapx;
void __cdecl SetWrapxHook(float fWrap)
{
    hbSetWrapx.fun(fDefaultCoords + fWrap * fDefaultWidth / CDraw::GetAspectRatio());
}
...
{
    hbDrawWindow.fun(CRect(fDefaultCoords + rect->left * fDefaultWidth / CDraw::GetAspectRatio(), rect->bottom, fDefaultCoords + rect->right * fDefaultWidth / CDraw::GetAspectRatio(), rect->top), titleKey, fadeState, color, a5, bDrawBox);
}
...
void __cdecl SetCentreSizeHook(float a1)
{
    ...
    {
        hbSetCentreSize.fun(a1 * fDefaultWidth / CDraw::GetAspectRatio());
    }
    else
...
void __cdecl DrawLoadingBarHook(float x, float y, unsigned int w, unsigned int h, float progress, int progressAdded, bool drawPercentage, bool drawBlackBorder, CRGBA const& color, CRGBA const& progressAddedColor)
{
    ...
    float fFrontendDefaultWidth = ((((float)RsGlobal->maximumHeight * (static_cast<float>(w) / static_cast<float>(h)))));

    x = ((float)RsGlobal->maximumWidth * 0.5f - fFrontendDefaultWidth * 0.5f + x * (4.0f / 3.0f) / CDraw::GetAspectRatio());
    w = static_cast<unsigned int>((float)w * (4.0f / 3.0f) / CDraw::GetAspectRatio());
...
public:
    Frontend2()
    {
        WFP::onGameInitEvent() += []()
        {
            ...
            {
                fWideScreenWidthScale = 640.0f / (CDraw::GetAspectRatio() * 448.0f);
                fWideScreenHeightScale = 448.0 / 448.0f;
                fWideScreenWidthProperScale = static_cast<float>(RsGlobal->maximumWidth) / (CDraw::GetAspectRatio() * 448.0f);

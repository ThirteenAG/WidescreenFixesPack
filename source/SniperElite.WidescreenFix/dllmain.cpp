...
DWORD WINAPI Init(LPVOID)
{
	...
	injector::WriteMemory(pattern.get(0).get<uint32_t>(16), Screen.Width, true);
	...
	}; injector::MakeInline<SetResHook>(pattern.get(0).get<uint32_t>(0), pattern.get(0).get<uint32_t>(12));
	...
	}; injector::MakeInline<SetResHook2>(pattern.get(0).get<uint32_t>(1), pattern.get(0).get<uint32_t>(9));
	injector::WriteMemory<uint8_t>(pattern.get(0).get<uint32_t>(0), 0x57, true); //push edi
	...
	if (bFixFOV)
	{
		...
		injector::WriteMemory(*pattern.get(0).get<uint32_t*>(2), fDynamicScreenFieldOfViewScale, true);
	}
	...
	if (bFixHUD)
	{
		...
		if (bFixText)
		{
			...
			}; injector::MakeInline<TextHook>(pattern.get(0).get<uint32_t>(0), pattern.get(0).get<uint32_t>(10));

			static int n0 = 0;
			pattern = hook::pattern("DB 05 ? ? ? ? 53 55 57 D9 5C 24 14"); //4D2166
			injector::WriteMemory(pattern.get(0).get<uint32_t>(2), &n0, true);
		}
	}
	return 0;
}

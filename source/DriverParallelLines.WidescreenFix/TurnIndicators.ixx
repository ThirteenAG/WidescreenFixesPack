...
struct CCharacter
{
    CVehicle* GetVehicle()
    {
        return *(CVehicle**)((uintptr_t)this + 0x688);
    }
};

struct CPlayerDriverBehaviour
{
    CCharacter* GetCharacter()
    {
        return *(CCharacter**)((uintptr_t)this + 0x2C4);
    }
};
...
struct CLifePlayer
{
    int field0;           // +0x00
    int field4;           // +0x04
    CPlayerDriverBehaviour* pDriverBehaviour; // +0x08
...
export GameRef<CLifeSystem*> pLifeSystem([]() -> CLifeSystem**
{
    auto pattern = hook::pattern("8B 0D ? ? ? ? 3B CF 89 3D");
    if (!pattern.empty())
        return *pattern.get_first<CLifeSystem**>(2);
    return nullptr;
});
...
export GameRef<CGameTime*> pGameTime([]() -> CGameTime**
{
    auto pattern = hook::pattern("A1 ? ? ? ? 8B 48 ? ? ? 8B 53");
    if (!pattern.empty())
        return *pattern.get_first<CGameTime**>(1);
    return nullptr;
});

class FrontendInputManager;
export GameRef<FrontendInputManager*> pFrontendInputManager([]() -> FrontendInputManager**
{
    auto pattern = hook::pattern("8B 0D ? ? ? ? 6A ? E8 ? ? ? ? 8B 0D ? ? ? ? 6A ? E8 ? ? ? ? 8B 0D ? ? ? ? 6A ? E8 ? ? ? ? 8B 0D ? ? ? ? 6A ? E8 ? ? ? ? A1");
    if (!pattern.empty())
        return *pattern.get_first<FrontendInputManager**>(2);
    return nullptr;
});
...
public:
    TurnIndicators()
    {
        WFP::onInitEvent() += []()
        {
            ...
            auto pattern = hook::pattern("55 8B EC 51 8B 51 ? ? ? 83 F8");
            ActivateLamp = (decltype(ActivateLamp))pattern.get_first();

            pattern = hook::pattern("C7 06 ? ? ? ? C7 46 ? ? ? ? ? C7 46 ? ? ? ? ? C7 46 ? ? ? ? ? C7 46 ? ? ? ? ? C7 07");
            ...
            WFP::onGameProcessEvent() += [&]()
            {
                ...
                if (ls)
                {
                    ...
                    if (p)
                    {
                        auto db = p->GetDriverBehaviour();
                        if (db)
                            PlayerPed = db->GetCharacter();
                    }
                }

                if (PlayerPed)
                {
                    PlayerCar = PlayerPed->GetVehicle();
                ...
                for (auto& pair : recent_vehicles)
                {
                    ...
                    if (player_in_vehicle && vehicleStruct)
                    {
                        ...
                        {
                            static DoubleTapDetector dtap_left;
                            static DoubleTapDetector dtap_right;
                            ...
                            if (ReadActionMapsID())
                            {
                                cur_lb = Natives::IsLBPressed();
                                cur_rb = Natives::IsRBPressed();
                            }
                        ...
                        else
                        {
                            ...
                            auto m_fSteerAngle = *(float*)((uintptr_t)vehicleStruct + 0xB4);

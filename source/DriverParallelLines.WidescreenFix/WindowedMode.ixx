...
public:
    FusionDxHook()
    {
        ...
        int32_t nWindowedMode = iniReader.ReadInteger("WINDOWEDMODE", "WindowedMode", 1);
        ...
        if (nWindowedMode)
        {
            ...
            //auto pattern = hook::pattern("88 1D ? ? ? ? FF 15 ? ? ? ? 8B F8");
            //injector::MakeNOP(pattern.get_first(), 6, true);
            //static auto WindowedMode = safetyhook::create_mid(pattern.get_first(), [](SafetyHookContext& regs)
            //{
            //    dwWindowedMode = 1;
            //});
        ...
        bForceWindowedMode = nWindowedMode > 0;

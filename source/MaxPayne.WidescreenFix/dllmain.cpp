...
DWORD WINAPI ComicsHandler(LPVOID)
{
    ...
    while (true)
    {
        ...
        if (bComicsMode)
        {
            if (*IsInComicsMode == 255 && *(uint8_t*)0x8A69E4) //+main menu check
            {
                if (!bPatched)
                {
...
DWORD WINAPI Thread(LPVOID)
{
    ...
    if (bUseGameFolderForSavegames)
        injector::WriteMemory<uint8_t>(0x40FCAB, 0x85, true);
    ...
    IsInComicsMode = (uint8_t*)((DWORD)GetModuleHandle("e2_d3d8_driver_mfc.dll") + 0x845E8);
    ...
    if (Screen.fAspectRatio > (4.0f / 3.0f))
    {
      ...
      //injector::WriteMemory((DWORD)e2mfc + 0x14775 + 0x2, &f1_fWidthScale, true);
        ...
        injector::WriteMemory((DWORD)e2mfc + 0x14775 + 0x2, &f1_fWidthScale, true);
        injector::WriteMemory<uint32_t>((DWORD)e2mfc + 0x14786, 0x000C00BB, true);
        injector::WriteMemory<uint8_t>((DWORD)e2mfc + 0x14786 + 4, 0x00, true);

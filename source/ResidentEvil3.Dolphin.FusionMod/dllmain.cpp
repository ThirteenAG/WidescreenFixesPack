            ...
            if (Dolphin::MemoryValid())
            {
                if (Dolphin::GameID() == "GLEE08"/* || Dolphin::GameID() == "GLEJ08"*/)
                {
                    ...
                    if (data == nullptr || (memcheck && *memcheck != bytes))
                    {
                        ...
                        auto pattern = hook::pattern(Dolphin::GameMemoryStart, Dolphin::GameMemoryEnd, "38 00 00 01 3c 60 ? ? 38 63 ? ? ? ? 00 00 3c 60");
                        if (pattern.size() > 1)
                        {
                        ...
                        if (bEnableDoorSkip)
                        {
                            pattern = hook::pattern(Dolphin::GameMemoryStart, Dolphin::GameMemoryEnd, "94 21 ff f0 7c 08 02 a6 ? ? ? ? ? ? ? ? 38 60 00 01");
                            if (pattern.size() > 1)
                            {
                    ...
                    {
                        while (bUnthrottleEmuDuringDoorSkip && Dolphin::MemoryValid(data) && data[0] == 1)
                        {

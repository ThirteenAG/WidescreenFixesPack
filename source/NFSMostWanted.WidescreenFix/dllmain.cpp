...
DWORD WINAPI Init(LPVOID)
{
    ...
    }; injector::MakeInline<RainDropletsHook>(pattern.get(0).get<uint32_t>(0), pattern.get(0).get<uint32_t>(8)); //6D480D
    ...
    }; injector::MakeInline<RainDropletsYScaleHook>(pattern.get(0).get<uint32_t>(30), pattern.get(0).get<uint32_t>(30 + 8)); //6D482B
    ...
    if (bRearviewMirrorFix)
    {
        ...
        injector::WriteMemory<uint8_t>((DWORD)dword_595DDA + 5, 4, true);

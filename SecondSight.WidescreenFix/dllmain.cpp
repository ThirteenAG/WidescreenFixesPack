...
DWORD WINAPI Init(LPVOID)
{
	...
	injector::WriteMemory(pattern.get(1).get<uint32_t>(2), &fARFactor, true); //0x43D26B
	
	pattern = hook::pattern("D8 0D ? ? ? ? 33 C9 D9 44 24 14 89 48 04 D8 64 24 10");
	injector::WriteMemory(pattern.get(0).get<uint32_t>(2), &Screen.fFieldOfView, true); //0x5E4B08
